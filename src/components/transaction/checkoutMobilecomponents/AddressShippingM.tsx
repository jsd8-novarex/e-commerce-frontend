...
const AddressShippingM: React.FC<AddressShippingDProps> = ({ handleContinue }) => {
  const { customer } = useCustomerProfile(); // Fetch customer data
  ...
  useEffect(() => {
    // Fetch addresses if customer ID exists
    if (customer?._id) {
  ...
  useEffect(() => {
    // Update state when addresses change
    if (addresses && addresses.length > 0) {
      const address = addresses[0]; // Use the first address in the list
      setEditAddress({
        ...
        {isEditing ? (
          ...
          <div className='flex justify-between border border-t-0 p-4'>
            <div className='flex flex-col'>
              ...
              <span>{`${editAddress.address}, ${editAddress.subdistrict}, ${editAddress.district}, ${editAddress.province} ${editAddress.postal_code}`}</span>
            </div>
            <div>
