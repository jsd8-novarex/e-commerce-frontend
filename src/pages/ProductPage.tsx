...
function ProductPage() {
  ...
  const filterData = ShowImageProductData2.filter((item)=> 
    item.product_choice_id === product_choice_id
  );
  const findProductChoice = product_list.find((item) =>
    item.product_choice.some((choice) => choice.id === product_choice_id),
  );

  const productChoiceData = findProductChoice?.product_choice.find((choice) => choice.id === product_choice_id)
